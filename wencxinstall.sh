...
if [ ! -f "/ql/data/config/config.yml" ]; then
 ...
 cp /ql/data/repo/TomyJan_CheckBox/config.yml.temple ./config.yml
else
 echo "已存在配置文件"
 echo "生成空白模板 ./config.yml.sample"
 cp /ql/data/repo/TomyJan_CheckBox/config.yml.temple ./config.yml.sample
fi
